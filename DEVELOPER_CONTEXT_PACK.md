...
> **Версия:** 2.2 | **Дата:** 2026-01-01 | **Target:** Onboarding разработчиков
...
- `authMiddleware` добавляет в `req.user`: `{ id, organizationId, departmentId, role, employeeId }`

### Row-Level Security (RLS) для водителей:
- **Склад:** Водители видят только свои баки ТС и топливные карты (`stockLocationController`, `stockBalanceController`)
- **Топливные карты:** Фильтрация по `assignedToDriver.employeeId`
- **Бланки:** Доступ к вкладке "Бланки ПЛ" при включённой настройке `driverCanAddBatches`
...
- **Проверка дубликатов:** При создании пачки проверяется пересечение диапазонов с существующими пачками той же серии

### 8.4 Настройки приложения (AppSettings)

| Настройка | Описание |
|-----------|----------|
| `isParserEnabled` | Включить парсер маршрутов из HTML |
| `appMode` | `driver` (упрощенный) / `central` (с проверкой) |
| `blanks.driverCanAddBatches` | Водитель может видеть вкладку "Бланки ПЛ" |
| `allowDeletePostedWaybills` | Разрешить удаление проведённых ПЛ |
| `allowDirectStockMovementDeletion` | Разрешить удаление складских движений |
...
### ⚠️ Проблема: Дублирование номеров бланков

**Причина:** Отсутствие валидации на пересечение диапазонов при создании пачки.

**Решение:** В `blankService.createBatch()` добавлена проверка:
```typescript
...
if (existingBatchOverlap) {
    throw new Error(`Диапазон пересекается с существующей пачкой`);
}
```
...
> **Примечание:** Этот документ актуален на 1 января 2026. При значительных архитектурных изменениях — обновлять документ.
