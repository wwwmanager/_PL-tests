...
### Проблема: Все endpoints возвращают 500 Internal Server Error

**Симптомы:**
- `/api/health` возвращает 500
- `/api/auth/login` возвращает 500
- Все запросы к базе данных падают

**Частые причины:**

1. **Zombie Node процессы блокируют порт 3001**
   
   Если вы несколько раз запускали/останавливали `npm run dev`, могли остаться висящие процессы.
...
2. **Prisma client не синхронизирован с schema.prisma**
   
   После изменения схемы или миграций:
   ```bash
   npx prisma generate
   npm run dev
   ```
...
3. **База данных недоступна**
   ...
   Проверьте `.env` файл:
   ```bash
   DATABASE_URL="postgresql://postgres:password@localhost:5432/waybills?schema=public"
   ```
   
   Убедитесь что PostgreSQL запущен и доступен.

4. **Битый Prisma client cache**
   ```bash
...
**Решение:**
См. пункт 1 выше — убейте все zombie Node процессы.

### Проблема: PowerShell тесты падают с ошибками JSON

**Причина:**
Inline JSON в PowerShell требует сложного экранирования кавычек.

**Решение:**
Используйте файлы для JSON payload:
```powershell
# Вместо inline JSON
curl.exe -d '{\"email\":\"admin@waybills.local\"}' # ❌

# Используйте файл
echo '{"email":"admin@waybills.local","password":"123"}' > login.json
curl.exe -d "@login.json" http://localhost:3001/api/auth/login # ✅
```
...
**Если миграции "pending":**
```bash
npx prisma migrate deploy
```

**Если схема расходится с БД:**
```bash
npx prisma db pull    # Посмотреть что в БД
...
- **[500 Error RCA](./docs/500_ERROR_RESOLUTION.md)** — полный анализ системной ошибки 500 из-за zombie процессов
- **[PR2 Final Verification](./docs/PR2_FINAL_VERIFICATION.md)** — проверка работоспособности после исправления
- **[Preventive Measures](./docs/PREVENTIVE_MEASURES_WALKTHROUGH.md)** — превентивные меры против повторения проблемы
- **[Prisma Documentation](https://www.prisma.io/docs/)** — официальная документация Prisma
- **[Express.js Guide](https://expressjs.com/)** — руководство по Express.js
