...
WB-101 — Prisma: добавить нормы расхода в Vehicle
Task
...
WB-102 — Prisma: добавить поля для планового расхода и флагов маршрута
Task
Добавить в БД:
...
BLS-101 — Prisma: индекс для быстрого выбора “следующего бланка”
Task
...
BLS-102 — Data audit: найти дубли blankId в waybills
Task
...
BLS-103 — Data fix: разрулить дубли blankId (safe)
Task
...
BLS-104 — Prisma: запретить один и тот же blankId в нескольких ПЛ
Task
...
WB-201 — Backend: расширить Vehicle API, чтобы отдавать/принимать fuelConsumptionRates
Task
...
WB-202 — Frontend: обновить vehicleApi.ts под fuelConsumptionRates
Task
...
WB-301 — Backend utils: реализовать isWinterDate под текущий SeasonSettings
Task
...
WB-302 — Backend domain: расчёт planned расхода и валидация топливного баланса
Task
...
WB-401 — WaybillController: DTO-валидация + legacy mapping топлива
Task
...
WB-402 — WaybillService: createWaybill — валидация одометра/топлива + расчёт fuelPlanned
Task
...
WB-403 — WaybillService: updateWaybill — replace fuelLines + запрет правок POSTED
Task
...
BLS-201 — BlankService: реализовать reserveNext/reserveSpecific/release/use/spoil атомарно
Task
...
BLS-202 — Интеграция: резервирование/освобождение бланка при create/update/delete Waybill
Task
...
WB-501 — changeWaybillStatus: транзакция + корректный перевод бланка и складских движений
Task
...
WB-601 — RBAC: разделить права на смену статусов (submit/post/cancel)
Task
...
WB-701 — Политика “факт > норма”: soft/hard validation + audit
Task
...
WB-801 — Тестовый набор: unit + integration для расчётов, бланков, статусов
Task
